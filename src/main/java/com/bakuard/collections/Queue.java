...
/**
 * <p>Реализация динамической очереди с объектами произвольного типа.</p>
 *
...
public sealed class Queue<T> implements ReadableLinearStructure<T> permits Deque {
	...
	/**
	 * <p>Создает и возвращает очередь, содержащую указанные элементы в указанном порядке.
	 * Длина создаваемого объекта ({@link #size()}) будет равна кол-ву передаваемых элементов.</p>
	 *
	 * <p>Итоговая очередь будет содержать копию передаваемого массива, а не сам массив.</p>
	 *
	 * <p>Если передаваемый массив не содержит ни одного элемента, то метод создает пустую очередь.</p>
	 *
	...
	/**
	 * <p>Удаляет элемент из начала очереди и возвращает его. Если очередь пуста, то возвращает null.</p>
	 *
	 * <p><b>ВАЖНО!</b> Т.к. очередь допускает хранение null элементов, то возвращение данным методом
	 * null в качестве результата не гарантирует, что очередь пуста. Для проверки наличия элементов
	 * в очереди используйте методы {@link #size()} или {@link #isEmpty()}.</p>
	 */
	...
	/**
	 * <p>Удаляет все элементы из очереди и уменьшает её длину до нуля.</p>
	 *
	 ...
	 * никаких изменений.</p>
	 *
	 * <p>Данный метод следует использовать в тех случаях, когда необходимо минимизировать объем памяти, занимаемый объектом Queue.</p>
	 *
